...
def upsert_card(supabase_url: str, supabase_key: str, payload: dict[str, Any]) -> None:
    base_url = f"{supabase_url.rstrip('/')}/rest/v1/cards"
    quoted_name = urllib.parse.quote(payload["card_name"])

    # Update existing row by card_name.
    patch_url = f"{base_url}?card_name=eq.{quoted_name}"
    supabase_request(
        "PATCH",
        patch_url,
        supabase_key,
    ...
    # Insert if not found.
    check_url = f"{base_url}?select=id&card_name=eq.{quoted_name}&limit=1"
    request = urllib.request.Request(
        check_url,
        headers={
            "apikey": supabase_key,
